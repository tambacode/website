...
add_filter( 'wp_setup_nav_menu_item', 'antreas_menu_fields' );
function antreas_menu_fields( $menu ) {
...
//Save the field data
add_action( 'wp_update_nav_menu_item', 'antreas_menu_fields_update', 10, 3 );
function antreas_menu_fields_update( $menu_id, $db_id, $args ) {
...
add_filter( 'wp_edit_nav_menu_walker', 'antreas_menu_fields_walker', 10, 2 );
function antreas_menu_fields_walker( $walker, $menu_id ) {
	return 'Antreas_Menu_Edit_Walker';
}


//Change the default walker for custom menu widgets
// add_filter('wp_nav_menu_args', 'antreas_menu_walker');
function antreas_menu_walker( $args ) {
	//Widgets do not use a theme_location
	if ( $args['theme_location'] == '' ) {
		$args = array_merge( $args, array( 'walker' => new Antreas_Menu_Walker() ) );
	}
	return $args;
}
